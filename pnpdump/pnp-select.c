...
alloc_resources ( struct resource *res, int count,
	...
	/*
	 * These next two lines should never occur now, but keep them to minimise
	 * infinite loops, just in case.
	 */
    if (res->type == IRQ_TAG && res->mask == 0) res->mask = 1;
    if (res->type == IOport_TAG && res->step == 0) res->step = 1;
